            ...
            for (int i = 0; i < nodes.Length; i++) {
                ...
                foreach (XNode.NodePort port in srcNode.Ports) {
                    for (int c = 0; c < port.ConnectionCount; c++) {
                        var connection = port.GetConnection(c);
                        if (connection == null)
                        {
                            --c;
                        }
                        XNode.NodePort inputPort = port.direction == XNode.NodePort.IO.Input ? port : connection;
                        XNode.NodePort outputPort = port.direction == XNode.NodePort.IO.Output ? port : connection;
